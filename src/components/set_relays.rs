        ...
        html! {
          ...
          <a class="block p-8 bg-white border border-gray-200 rounded-lg shadow-lg hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 sm:w-full lg:w-1/2">
            <div class="relative z-0 w-full mb-8 group">
                <input type="text" name="mint_url" id="mint_url" class="block py-4 px-6 w-full lg:text-lg sm:text-5xl text-gray-900 bg-transparent border-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:border-blue-600 peer" placeholder={"Relay Url"} ref={self.relay_node_ref.clone()} />
              <div class="flex justify-center mt-8">
                <button class="px-6 py-2 mt-2 rounded-sm shadow-sm dark:bg-violet-400 dark:text-gray-900 lg:text-lg sm:text-5xl" onclick={set_relay}>{"Add Relay"}</button>
                <button class="px-6 py-2 mt-2 rounded-sm shadow-sm dark:bg-violet-400 dark:text-gray-900 lg:text-lg sm:text-5xl" onclick={ctx.props().relays_set_cb.clone()}>{"Next"}</button>
              </div>
            </div>
          </a>
        </div>
